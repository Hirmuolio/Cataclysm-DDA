...
{
    // Battery charges from old batteries go into corpse pocket (probably something about pocket indexes).
    std::list<item *> items = all_items_top( item_pocket::pocket_type::CORPSE );
    if( items.empty() ) {
        // Already migrated
        return;
    } else if( items.size() > 1 ) {
        debugmsg( "Failed to migrating old battery %s.  It has many items in its corpse pocket.", tname() );
        return;
    }
